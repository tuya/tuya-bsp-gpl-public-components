...
static int codec_dac_open(SND_PARAM *codec)
{
	...
	reg_val = __raw_readl(codec->sysctrl_base + DAC_ADC_AUDDIO_CLK_REG);
	reg_val |= (1 << CLOCK_SDDAC_CLK_EN_CFG_SHIFT);
	__raw_writel(reg_val,codec->sysctrl_base + DAC_ADC_AUDDIO_CLK_REG);

	reg_val = __raw_readl(codec->sysctrl_base + HIGHSPEED_CLOCK_CTRL_REG);
	reg_val |= (1 << CLOCK_SDDAC_HS_CLK_EN_CFG_SHIFT);
...
static int codec_dac_close(SND_PARAM *codec)
{
	...
	reg_val &= (~DAC_CTRL_EN);
	__raw_writel(reg_val,codec->adda_cfg_base + DAC_CTRL_CFG_REG);
	//close dac controller clk
	reg_val = __raw_readl(codec->sysctrl_base + CLOCK_GATE_CTRL_REG);
	reg_val |= (1<<4);
	__raw_writel(reg_val,codec->sysctrl_base + CLOCK_GATE_CTRL_REG);
...
static int codec_adc_open(SND_PARAM *codec)
{
	...
	reg_val = __raw_readl(codec->sysctrl_base + DAC_ADC_AUDDIO_CLK_REG);
	reg_val |= (1 << CLOCK_SDADC_CLK_EN_CFG_SHIFT);
	__raw_writel(reg_val,codec->sysctrl_base + DAC_ADC_AUDDIO_CLK_REG);

	reg_val = __raw_readl(codec->sysctrl_base + HIGHSPEED_CLOCK_CTRL_REG);
	reg_val |= (1 << CLOCK_SDADC_HS_CLK_EN_CFG_SHIFT);
	__raw_writel(reg_val,codec->sysctrl_base + HIGHSPEED_CLOCK_CTRL_REG);
...
int dac_close(void)
{
	...
	codec_dac_close(codec);

	dac_clk_disable(codec);
...
static void set_hp_in(unsigned long signal)
{
	...
	/*
	* NOTE:SUN4 DONOT SUPPORT LINEIN.
	*/
...
int adc_close(void)
{
	...
	codec_adc_close(codec);

	adc_clk_disable(codec);
