...
module_param(cpu_mode, int, 0644);
#endif

#define HDBG(stuff...) pr_debug("USBHS: " stuff)
#define VDBG	HDBG

static int period_epfifo;
struct workqueue_struct *g_otghc_wq;
struct delayed_work	g_otg_rest;
...
void port_power(struct akotg_usbhc *akotghc, int is_on)
{
	...
	if (is_on) {
		HDBG("%s_%d,akotghc->port_status=%x,hcd->state =%x\n",__func__, __LINE__,akotghc->port_status,hcd->state);
...
//#ifdef CONFIG_USB_AKOTG_DMA
#if 0
        if(!cpu_mode && (len == 512) && (remain >= len))
        {
...
#ifdef CONFIG_USB_AKOTG_DMA
	HDBG("Packet: OUT Packet (EP %d), Length=%d,cpu_mode=%d\n", ep->epnum, urb->transfer_buffer_length - urb->actual_length,cpu_mode);
#else
...
static inline void start_transfer_epx(struct akotg_usbhc *akotghc, int epfifo)
{
	...
	if (akotghc->active_epx[epfifo-1] == NULL) {
		...
		VDBG("S_ep%d=%p\n", epfifo, akotghc->active_epx[epfifo-1]);
		if (akotghc->active_epx[epfifo-1] != NULL)
...
done(struct akotg_usbhc *akotghc, struct akotghc_ep *ep)
{
	...
	if (err_occurred) {
		pr_err("@done, epfifo=%x, is_out=%x,urbstat=%d\n", epfifo,is_out,urbstat);
		if (epfifo == 0)
...
static void reset_otg(struct work_struct *work)
{
	...
	/* USB_sus, 1 = suspend*/
	con |= (0x3 << 1);
	/*USB_PHY_rst, 1 = To reset the transceiver of USB 2.0 HS controller.*/
	con |= (1 << 0);
	__raw_writel(con, USB_OP_MOD_REG);
	set_usb_as_host();	
	con = __raw_readl(USB_OP_MOD_REG);
	/*USB_PHY_rst, 0 = No instruction.*/
	con &= ~(0x01);  
	__raw_writel(con, USB_OP_MOD_REG);
	...
	hc_writeb(0xF7, USB_REG_INTRUSBE);
		...
		/*
		 * IP GOAL 提出解决1.0V不能正常上传的方案
		 * 设置 USB_RREF_CON_REG : 0x08000034 的[30:31] = 2'b11
		 */
		con = __raw_readl(USB_RREF_CON_REG);
		con |= (0x3<<30);
		__raw_writel(con, USB_RREF_CON_REG);
#endif
	...
	for(i=0; i<MAX_EP_NUM; i++){
		if(rINTTX & (1<<(i+1))) {
			...
			if(ep && !ep->bdma){
				...
				VDBG("D_ep%d=%p ", i+1,akotghc->active_epx[i]);
		...
		for(i=0; i<MAX_EP_NUM; i++) 
			if (akotghc->active_epx[i]) {
				...
				if (usb_pipeout(urb->pipe)) {
					...
					/* 
					 * With Index Register Value not set to 0 in HOST mode:(Address: 0x2020,0012) 
					 * Bit[3]
					 * 
					 * flushfifo
					 * 	0: No instruction
					...
					/* 
					 * With Index Register Value not set to 0  In HOST mode:(Address: 0x2020,0016)
					 * Bit[4]
					 * 
					 *	flushfifo
					 *	0: No instruction.
...
) 
{
	...
	if (!__is_epnum_mapped(&akotg_epfifo_mapping, epnum, is_out)) {
		...
		HDBG("###EPNum%d_%s mapping Host EPFIFO %d \n",epnum, is_out ? "OUT" : "IN", epfifo);
		if (epnum != 0) {
	...
	if (!(akotghc->port_status & (1 << USB_PORT_FEAT_ENABLE))
		...
		retval = -ENODEV;
		HDBG("akotghc->port_status=%x, hcd->state=%x\n",akotghc->port_status, hcd->state); //akotghc->port_status=503, hcd->state=0
		HDBG("ep%d dead or disabled port!\n",epnum); //USBHS: ep0 dead or disabled port!
		goto fail_not_linked;
	}

	retval = usb_hcd_link_urb_to_ep(hcd, urb);
	if (retval) {
		HDBG("EPNum%d usb_hcd_link_urb_to_ep fail!, retval=%d\n",epnum,retval);
		goto fail_not_linked;
	}
...
akotg_usbhc_timer(unsigned long _akotghs)
{
	...
	if (akotghc->port_status & USB_PORT_STAT_RESET) {
		...
		if (akotghc->port_status & (1 << USB_PORT_FEAT_CONNECTION)) {
			...
			/*
			 * support low speed device in 37D
			 * bDeviceProtocol of usb2_rh_dev_descriptor must be 0x01 (usb 2.0 Single TT) in linux/drivers/usb/core/hcd.c
			 */
				akotghc->port_status |= USB_PORT_STAT_LOW_SPEED;
			#else
...
int akotg_usbhc_reset(struct usb_hcd *hcd)
{
	...
	/* PHY_OTG_SUSPENDM,
	 * bit1
	 * 0= To use approximate detection circuit, the consumption current is about 50uA
	 * 1= To use accurate detection circuit, the consumption current is about 400uA
	 */
	con |= (0x3 << 1);
	/* USB_PHY_rst, bit0, 1 = To reset the transceiver of USB 2.0 HS controller*/
	con |= (1 << 0);
	__raw_writel(con, USB_OP_MOD_REG);
