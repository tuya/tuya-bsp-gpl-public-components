...
*          CMT2300A_STA_CAL
* *********************************************************/
BYTE CMT2300A_GetChipStatus(void)
{
...
* *********************************************************/
BOOL CMT2300A_AutoSwitchStatus(BYTE nGoCmd)
{
#ifdef ENABLE_AUTO_SWITCH_CHIP_STATUS
    DWORD nBegTick = CMT2300A_GetTickCount();
    BYTE  nWaitStatus;
	DWORD times = 0;
    ...
    while(CMT2300A_GetTickCount()-nBegTick < 10)
    {
		...
		if ((times++) > 20000)		//subG模块有问题时直接等2秒后返回错误
			return FALSE;
    }
...
* *********************************************************/
void CMT2300A_ConfigInterrupt(BYTE nInt1Sel, BYTE nInt2Sel)
{
...
{
    BYTE tmp = CMT2300A_ReadReg(CMT2300A_CUS_INT1_CTL);
...
* *********************************************************/
void CMT2300A_SetFifoThreshold(BYTE nFifoThreshold)
{ 
    BYTE tmp = CMT2300A_ReadReg(CMT2300A_CUS_PKT29);
...
* *********************************************************/
void CMT2300A_EnableAntennaSwitch(BYTE nMode)
{
    BYTE tmp = CMT2300A_ReadReg(CMT2300A_CUS_INT1_CTL);
...
* *********************************************************/
void CMT2300A_EnableInterrupt(BYTE nEnable)
{
...
{
    BYTE tmp = CMT2300A_ReadReg(CMT2300A_CUS_FIFO_CTL);
...
* *********************************************************/
BYTE CMT2300A_ClearTxFifo(void)
{
    BYTE tmp = CMT2300A_ReadReg(CMT2300A_CUS_FIFO_FLAG);
...
*            CMT2300A_MASK_TX_FIFO_TH_FLG
* *********************************************************/
BYTE CMT2300A_ClearRxFifo(void)
{
    BYTE tmp = CMT2300A_ReadReg(CMT2300A_CUS_FIFO_FLAG);
...
* *********************************************************/
BYTE CMT2300A_ClearInterruptFlags(void)
{
    BYTE nFlag1, nFlag2;
    BYTE nClr1 = 0;
    BYTE nClr2 = 0;
...
* *********************************************************/
void CMT2300A_ConfigTxDin(BYTE nDinSel)
{
    BYTE tmp = CMT2300A_ReadReg(CMT2300A_CUS_FIFO_CTL);
...
{
    BYTE tmp = CMT2300A_ReadReg(CMT2300A_CUS_INT2_CTL);
...
* @return  RSSI code
* *********************************************************/
BYTE CMT2300A_GetRssiCode(void)
{
...
* *********************************************************/
void CMT2300A_SetFrequencyChannel(BYTE nChann)
{
...
* *********************************************************/
void CMT2300A_SetFrequencyStep(BYTE nOffset)
{
...
* @param   nLength
* *********************************************************/
void CMT2300A_SetPayloadLength(WORD nLength)
{ 
    BYTE tmp = CMT2300A_ReadReg(CMT2300A_CUS_PKT14);
...
{
    BYTE tmp = CMT2300A_ReadReg(CMT2300A_CUS_SYS2);
...
{
    BYTE tmp = CMT2300A_ReadReg(CMT2300A_CUS_FSK5);
...
* *********************************************************/
void CMT2300A_SetAfcOvfTh(BYTE afcOvfTh)
{
...
* *********************************************************/
BOOL CMT2300A_ConfigRegBank(BYTE base_addr, const BYTE bank[], BYTE len)
{
    BYTE i;
    for(i=0; i<len; i++)
