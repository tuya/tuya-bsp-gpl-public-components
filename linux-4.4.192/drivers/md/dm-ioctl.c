...
static int dev_create(struct dm_ioctl *param, size_t param_size)
{
	...
	if (r)
	{
		DMERR("%s %d dm_create fail!\n", __func__, __LINE__);
		return r;
	}

	r = dm_hash_insert(param->name, *param->uuid ? param->uuid : NULL, md);
	if (r) {
		DMERR("%s %d dm_hash_insert fail!\n", __func__, __LINE__);
		dm_put(md);
		dm_destroy(md);
		return r;
	}
			   ...
			   char **target_params_array)
{
	...
	if (r)
	{
		DMERR("%s %d dm_table_create fail!\n", __func__, __LINE__);
	...
	for (i = 0; i < dmi->target_count; i++) {
		...
		if (r) {
			printk("error adding target to table\n");
			goto err_destroy_table;
		}
	}
	...
	if (r)
	{
		DMERR("%s %d dm_table_complete fail!\n", __func__, __LINE__);
		goto err_destroy_table;
	}
	...
	dm_unlock_md_type(md);// add by hs
	...
	if (r)
	{
		DMERR("%s %d dm_resume fail!\n", __func__, __LINE__);
		goto err_destroy_table;
	}

	DMINFO("(%s) is ready", dmi->name);
	dm_put(md);
	return 0;
