...
{
	struct cdc_state	*info = (void *) &dev->data;
	//struct usb_cdc_notification notification;	//change for 4G module: disable interrupt transfer
	int			master_ifnum;
	int			retval;
	//int			partial;	//change for 4G module: disable interrupt transfer
	unsigned		count;
...
#if 0  // change for 4G module: disable interrupt transfer
...
#else
	mdelay(200) ; //wait for a short time for ready
#endif
