...
__rmqueue_fallback(struct zone *zone, unsigned int order, int start_migratetype)
{
	...
	for (current_order = MAX_ORDER-1;
		...
		page = list_entry(area->free_list[fallback_mt].next,
						struct page, lru);
		if (can_steal  &&  get_pageblock_migratetype(page) != MIGRATE_HIGHATOMIC)
...
retry:
					 ...
					 *	is in the CMA area.
					 */
					if (migratetype == MIGRATE_MOVABLE && zone_page_state(zone, NR_FREE_CMA_PAGES) > zone_page_state(zone, NR_FREE_PAGES) / 2) {
