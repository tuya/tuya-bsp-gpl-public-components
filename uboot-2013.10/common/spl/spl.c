...
char g_env_buf[CONFIG_ENV_SIZE] = {0};
...
void board_init_r(gd_t *dummy1, ulong dummy2)
{
	...
	env_relocate();
#else
	flash_size = env_relocate_spec_spl(g_env_buf);
#endif

#ifndef CONFIG_37_E_CODE//37E IN BOOT CODE READ EFUSE
	efuse_val = efuse_read();
	//printf("efuse_read:0x%08lx\n", efuse_val);
#endif

	if(gd->env_valid){
		memset(out_buf, 0, 1024);
		pare_str_form_env(g_env_buf, "loadaddr=", out_buf);
		...
		memset(out_buf, 0, 1024);
		pare_str_form_env(g_env_buf, "mtdparts=mtdparts=", out_buf);
		part_num = spl_device_parse(&out_buf[9], flash_size);
	}
	else{
