...
int fw_env_close(void)
{
	 ...
	 * Update CRC
	 */
	fprintf(stderr,"===Make Environment Image Success!===\n");
	*environment.crc = crc32(0, (unsigned char *) environment.data, ENV_SIZE);
 ...
 * < 0	- failed to test
 */
static int flash_bad_block (int fd, unsigned char mtd_type, loff_t *blockstart)
{
...
static int flash_read_buf (int dev, int fd, void *buf, size_t count,
			   off_t offset, unsigned char mtd_type)
{
...
int fw_env_open(void)
{
	...
	fprintf(stderr,"===Make Environment Image Start===\n");
	...
	if (!HaveRedundEnv) {
		if (!crc0_ok) {
			...
			fprintf(stderr,"The env size for environment(%ld bytes)\n",ENV_SIZE);
			memset(environment.data, 0x00, ENV_SIZE);
		}
	} else {
		...
		crc1 = crc32 (0, (unsigned char *) redundant->data, ENV_SIZE);
